...
final class AccountService
{
    ...
    public function ensureAdminPayableAccount(int $adminId): int
    {
        $pdo = Database::pdo();

        $stmt = $pdo->prepare('SELECT id FROM treasury_accounts WHERE admin_id = :admin_id AND account_type = "liability" AND code LIKE "2000:%" LIMIT 1');
        $stmt->execute(['admin_id' => $adminId]);
        $existing = $stmt->fetchColumn();
        if ($existing) {
            return (int)$existing;
        ...
        $parentId = $this->accountIdByCode('2000');
        $code = '2000:' . $adminId;
        $name = 'Funds Owed to Admin - ' . ($admin['display_name'] ?: $admin['rsn']);

        $insert = $pdo->prepare(
            'INSERT INTO treasury_accounts
             (code, name, account_type, parent_account_id, admin_id, normal_balance, is_system, is_active)
             VALUES
             (:code, :name, "liability", :parent_id, :admin_id, "credit", 1, 1)'
        );
        $insert->execute([
            'code' => $code,
            'name' => $name,
